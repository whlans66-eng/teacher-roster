    ...
    handler: async (request, context) => {
        ...
        try {
            ...
            const user = await validateRequest(authHeader, {
                onTiming: (step, details) => {
                    const fields = Object.entries(details || {})
                        .map(([key, value]) => `${key}=${value}`)
                        .join(' ');
                    context.log(`[permissions-check] step=${step} ${fields}`);
                }
            });

            if (!user) {
                context.log(`[permissions-check] completed durationMs=${Date.now() - startedAt} authenticated=false`);
                return {
                    status: 401,
                    headers,
            ...
            context.log(
                `[permissions-check] completed durationMs=${Date.now() - startedAt} ` +
                `email=${user.email} allowed=${user.allowed} isAdmin=${user.isAdmin} isSuperAdmin=${user.isSuperAdmin}`
            );
